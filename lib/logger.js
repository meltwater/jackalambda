 ...
 * @param {Object} [options] - Additional options to pass to the logger
 * @returns {Object} - A pino compatible logger with settings and a base context filled in
 */
export function createLogger(preCtx = {}, t = undefined, options = {}) {
  return createMlabsLogger({
    name,
    level,
    t,
    outputMode: isDev ? 'pretty' : 'json',
    base: { ...createBase(preCtx), ...options.base },
    ...options
  })
}
