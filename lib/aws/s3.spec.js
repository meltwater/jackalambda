...
test('listObjectVersions should return payload', async (t) => {
  const { createClient } = t.context
  const client = createClient(t)
  const promise = td.function()

  td.when(client._s3.listObjectVersions({})).thenReturn({
    promise
  })
...
test('deleteObjects should return payload', async (t) => {
  const { createClient } = t.context
  const client = createClient(t)
  const promise = td.function()

  td.when(client._s3.deleteObjects({})).thenReturn({
    promise
  })
