...
export class S3Client {
  ...
  async getObjectJson(key, params = {}) {
    ...
    try {
      ...
      if (key.match(/.+\.json$/g) == null) {
        throw new Error(`Reject non json Key: ${key}`)
      }
      ...
      const dataString = res.Body.toString('utf-8')
      const data = dataString.length ? JSON.parse(dataString) : null
