...
export class EventbridgeClient {
   ...
   * @param {Object} payload - The events to pass to eventbridge.Events. Event input structure is identical to the AWS Event type, except all keys are in lower camelcase, the Detail should be a plain serializable object, and the EventBusName should be omitted.
  ...
  async putEvents(events = [], params = {}) {
    ...
    try {
      ...
      const req = {
        Events: events.map((event) => this._toEventRequest(event)),
        ...params
      }
