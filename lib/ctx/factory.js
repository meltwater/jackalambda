 ...
 * @param {Object} [loggerOptions] - Addtional options to pass to the logger
 *
 * @returns {AppContext} - The new context
 */
export function createCtx(event, context, t, loggerOptions) {
  try {
    const preCtx = createPreCtx(event, context)
    const log = createLogger(preCtx, t, loggerOptions)
